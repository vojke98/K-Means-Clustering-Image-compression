...
void kmeans_openmp(unsigned char *imageIn, unsigned char *imageOut, int width, int height, int pitch, int clusters, int iterations);
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 6) {
        printf("USAGE: ./KMeansClustering input_image output_image number_of_clusters number_of_iterations number_of_threads\n");
        //exit(EXIT_FAILURE);
        exit(EXIT_SUCCESS);
    }
    ...
    printf("Width=%d; \tHeight=%d; \tPitch=%d; \tBPP=%d\n", width, height, pitch, bpp);
    ...
    kmeans_openmp(imageIn, imageOut, width, height, pitch, clusters, iterations);
